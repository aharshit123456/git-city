...
export async function GET(request: Request) {
  ...
  let from = Math.max(0, parseInt(searchParams.get("from") ?? "0", 10));
  let to = parseInt(searchParams.get("to") ?? "1000", 10);
  const targetUser = searchParams.get("user");

  const sb = getSupabaseAdmin();

  // If targetUser is provided, find their rank and adjust the range
  if (targetUser) {
    const { data: userDev } = await sb
      .from("developers")
      .select("rank")
      .eq("github_login", targetUser)
      .maybeSingle();

    if (userDev && userDev.rank !== null) {
      const rank = userDev.rank;
      const chunkSize = 1000;
      from = Math.floor((rank - 1) / chunkSize) * chunkSize;
      to = from + chunkSize;
    }
  }
